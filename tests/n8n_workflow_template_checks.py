            ...
            with self.subTest(display=display):
                ...
                for command in relevant:
                    ...
                    if country == "cn":
                        ...
                        self.assertIn("fetch origin master:refs/remotes/origin/master", command)
        ...
        for display, (country, _) in EXPECTED.items():
            with self.subTest(display=display):
                ...
                for command in relevant:
                    ...
                    self.assertIn('"${REPAIR_PROCESS_MAX_SECONDS}s"', command)
                    self.assertIn("REPAIR_PROCESS_MAX_SECONDS=", command)
